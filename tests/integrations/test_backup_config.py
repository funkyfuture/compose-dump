...
def test_yet_another_blog(project_dir, temp_dir, target, compression, resolve_symlinks):
    ...
    if compression:
        ...
        with TemporaryDirectory() as tmp:
            ...
            assert identical_folder_contents(
                project_dir, Path(tmp) / "config", resolve_symlinks
            )
    else:
        assert target_item.is_dir()
        target_folder = target_item
        assert identical_folder_contents(
            project_dir, target_folder / "config", resolve_symlinks
        )
